...
class SetupLocalCommand extends Command
{
	...
	public function handle()
	{
		...
		if ($this->confirm('>> Would you like to setup a mysql database connection interactively?', true)) {
			$this->updateEnv($updatedData = $this->requestDatabaseCredentials());
		} else {
			$this->warn('<warn> Make sure the .env file is configured correctly before proceeding.');
		...
		exec('php artisan db:make');

		try {
			$this->migrateDatabase($updatedData ?? null);
			$this->seedDatabase();
